...
class ContractRecurringMixin(models.AbstractModel):
    ...
    def _compute_is_deferred(self):
        """
        No depends on company setting: it's only for default value at create
        Section lines (display_type) should never be deferred
        """
        for rec in self:
            rec.is_deferred = (
                not rec.display_type and rec.company_id.defer_contract_line_start
            )
